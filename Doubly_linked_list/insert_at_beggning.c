...
struct node *insert_at_beggining(struct node *start)
{
	struct node *newnode ;
	newnode=(struct node *)malloc(sizeof(struct node ));
	printf("enter the data you want to insert at the beggining of the list:\n");
	scanf("%d",&newnode->data);
	newnode->next=NULL;
	newnode->prev=NULL;
	start->prev=newnode;
	newnode->next=start;
	start=newnode;
	printf("successfully inserted at the beggining \n");
	return start;
}
...
int main ()
{
	...
	new_head=insert_at_beggining(head);
	display(new_head);
	return 0;
}	
